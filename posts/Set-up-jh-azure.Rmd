...
See examples of full `config.yaml` files in the `config-template` directory in the [nmfs-opensci/nmfs-jhub](https://github.com/nmfs-opensci/nmfs-jhub) GitHub repo.
...
It is very easy to get lost in your names and it is tempting to use the same name over and over, but you will regret that.

* **Kubernetes cluster** `jhub`
* **Resource group** `SAFSJHub`
* **daskhub namespace** `daskhub`
* **

## Open the Azure Cloud Shell

I am using a simpler version of the instructions here: [Z2JH Set up Kubernetes on Azure](https://z2jh.jupyter.org/en/stable/kubernetes/microsoft/step-zero-azure.html). There is a step that requires that you have Owner role on your Azure subscription. I am using a subscription via a university and I don't have that. Also I will just accept most of the Azure defaults.
...
az aks nodepool add \
    ...
    --zones 2
```
* `--name usersmall`  give this node pool a unique name. We will need this in our JHub config file.
...
* `--labels hub.jupyter.org/node-purpose=usersmall` So that we can decipher our node and pod output.
...
az aks nodepool add \
    ...
    --zones 2
```
* `--node-count 2` The default is 3 and it threw an error if I didn't pass this in.
...
### Add the storage class to the config
